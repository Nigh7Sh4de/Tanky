/*
 PlayCanvas Engine v0.164.0-dev
 revision 3ff0b1a
...
this._viewMat);this._viewProjMat.transformPoint(a,d);a=a.data;g=this._viewProjMat.data;a=a[0]*g[3]+a[1]*g[7]+a[2]*g[11]+1*g[15];d.x=0.5*(d.x/a+1)*b;d.y=0.5*(1-d.y/a)*c;return d},screenToWorld:function(a,b,c,d,g,e){void 0===e&&(e=new pc.Vec3);var k=this.getProjectionMatrix(),l=this._node.getWorldTransform();this._viewMat.copy(l).invert();this._viewProjMat.mul2(k,this._viewMat);k=this._viewProjMat.clone().invert();this._projection===pc.PROJECTION_PERSPECTIVE?(b=new pc.Vec3(2*(a/d)-1,2*((g-b)/g)-1,1),
a=k.transformPoint(b),a.scale(1/(b.x*k.data[3]+b.y*k.data[7]+b.z*k.data[11]+k.data[15])),c/=this._farClip,e.lerp(this._node.getPosition(),a,c)):(c=new pc.Vec3(2*(a/d)-1,2*((g-b)/g)-1,2*((this._farClip-c)/(this._farClip-this._nearClip))-1),k.transformPoint(c,e));return e},getAspectRatio:function(){return this._aspect},getClearOptions:function(){return this._clearOptions},getFarClip:function(){return this._farClip},getFov:function(){return this._fov},getFrustum:function(){return this._frustum},getNearClip:function(){return this._nearClip},
...
a.name;for(var c=0;c<a.parameters.length;c++){var d=a.parameters[c];if("vec3"===d.type)this[d.name]=new pc.Color(d.data[0],d.data[1],d.data[2]);else if("vec2"===d.type)this[d.name]=new pc.Vec2(d.data[0],d.data[1]);else if("texture"===d.type){var g=d.name;if(d.data)if(d.data instanceof pc.Texture)d=d.data;else throw Error("PhongMaterial.init() expects textures to already be created");else d=null;this[g]=d}else"bumpMapFactor"===d.name?this.bumpiness=d.data:this[d.name]=d.data}this.update()},_updateMapTransform:function(a,
c,d){a=a||new pc.Vec4;a.set(c.x,c.y,d.x,d.y);return 1==a.x&&1==a.y&&0==a.z&&0==a.w?null:a},_collectLights:function(a,c,d){for(var g=0;g<c.length;g++)c[g].getEnabled()&&c[g].getType()==a&&d.push(c[g])},_updateMap:function(a){a+="Map";if(this[a]){this.setParameter("texture_"+a,this[a]);var c=a+"Transform";this[c]=this._updateMapTransform(this[c],this[a+"Tiling"],this[a+"Offset"]);this[c]&&this.setParameter("texture_"+c,this[c].data)}},update:function(){this.clearParameters();this.ambientUniform[0]=
...
e._super.initializeComponentData.call(this,a,b,c)},onRemove:function(a,b){b.model&&(this.context.scene.removeModel(b.model),a.removeChild(b.model.getGraph()),b.model=null)}});return{SkyboxComponentSystem:e}}());pc.extend(pc,function(){var e=function(){this.on("set",this.onSet,this)},e=pc.inherits(e,pc.Component);pc.extend(e.prototype,{onSet:function(b,c,d){function e(b,c){if(!(void 0===c||null===c)){var d=a.indexOf(b),g=this.entity.skybox.assets;this.data.model=null;g[d]=this.system.context.assets.getAssetById(c);if(g[d]){if(this.data.assets=g,g[0]&&g[1]&&g[2]&&g[3]&&g[4]&&g[5]){d=this.data;a:{for(var f=this.entity,h=this.system.context,k=h.graphicsDevice,l=[],v=[],A=[],x,z=0;z<g.length;z++)if(x=g[z])if(x.resource){x=
x.resource.getSource();if(!x){logERROR(pc.string.format("Trying to load skybox component with invalid texture types"));g=void 0;break a}l.push(x)}else A.push(z),v.push(new pc.resources.TextureRequest(x.getFileUrl()));else{logERROR(pc.string.format("Trying to load skybox component before assets are loaded"));g=void 0;break a}var C=new pc.Texture(k,{format:pc.PIXELFORMAT_R8_G8_B8,cubemap:!0});C.minFilter=pc.FILTER_LINEAR_MIPMAP_LINEAR;C.magFilter=pc.FILTER_LINEAR;C.addressU=pc.ADDRESS_CLAMP_TO_EDGE;
C.addressV=pc.ADDRESS_CLAMP_TO_EDGE;v.length?(g={parent:f.getRequest()},h.loader.request(v,g).then(function(a){for(var b=0;b<a.length;b++)l[A[b]]=a[b].getSource();C.setSource(l)})):C.setSource(l);v=new pc.Material;v.updateShader=function(){var a=k.getProgramLibrary().getProgram("skybox",{hdr:!1,gamma:h.scene.gammaCorrection,toneMapping:h.scene.toneMapping});this.setShader(a)};v.updateShader();v.setParameter("texture_cubeMap",C);v.cull=pc.CULLFACE_NONE;g=new pc.GraphNode;f=pc.createBox(k);v=new pc.MeshInstance(g,
f,v);f=new pc.Model;f.graph=g;f.meshInstances=[v];g=f}d.model=g;this.enabled&&this.entity.enabled&&(this.system.context.scene.addModel(this.data.model),this.entity.addChild(this.data.model.graph))}}else logERROR(pc.string.format("Trying to load skybox component before asset {0} has loaded",c))}}if("cubemap"==b&&d){var b=this.data,f=this.system.context,k=f.graphicsDevice,l=new pc.Material;l.updateShader=function(){var a=k.getProgramLibrary().getProgram("skybox",{hdr:d.hdr,prefiltered:!0,gamma:f.scene.gammaCorrection,
